...
public class CategoryThreadActivity extends Fragment  {
    ...
    private Runnable loadThreads = new Runnable() {
        public void run() {
            ++block;
            try {
                dataChunk = ControllerForumPresentation.getUniqueInstance().getThreadsCategory(category, block, sortedByVotes);
    ...
    private Runnable loadThreadsSearch = new Runnable() {
        public void run() {
            ++block;
            dataChunk = ControllerForumPresentation.getUniqueInstance().getThreadsCategorySearch(category, block, sortedByVotes, searchWords);
            UIUpdater.sendEmptyMessage(0);
        }
    };
